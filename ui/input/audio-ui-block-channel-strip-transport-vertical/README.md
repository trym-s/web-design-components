# Channel Strip Transport Vertical
...
- Framework: react
- Entry point: `upstream/examples/block-channel-strip-transport-vertical.tsx`
...
- Provides: Channel Strip Transport Vertical block
- Requires: React, Tailwind v4 with the shadcn tokens and a shadcn style (`cn-*` slots), `@audio-ui/react` 0.1.2
- Variants: block-channel-strip-transport-vertical
- Upstream: Audio UI · channel-strip, transport
- Preferred install: `npx shadcn@latest add @audio/block-channel-strip-transport-vertical`
...
- `upstream/examples/block-channel-strip-transport-vertical.tsx` — Channel strip with transport seek bar in vertical orientation · static: `static/block-channel-strip-transport-vertical.html`
...
### BlockChannelStripTransportVertical — `block-channel-strip-transport-vertical.tsx`

- Props: same as `BlockChannelStripTransport`.
- Layout: The same with the seek bar laid out in a vertical strip (the bar itself stays horizontal).
