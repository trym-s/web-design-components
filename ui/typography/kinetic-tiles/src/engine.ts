...
export interface KineticParams {
  ...
  fontFamily?: string; // display face for the letter (weight 700); default system-ui 800
}

// ── small colour helpers ────────────────────────────────────────────────────
/** Any CSS colour → sRGB 0–255 via a 1×1 canvas (the params are CSS colours, e.g. resolved tokens). */
...
const rgb = ([r, g, b]: [number, number, number], a = 1) => `rgb(${r} ${g} ${b} / ${a})`;
...
export class KineticA {
  ...
  // set from the params in applyColors()
  private paperRGB: [number, number, number] = [0, 0, 0];
  private inkRGB: [number, number, number] = [0, 0, 0];
  private accentRGB: [number, number, number] = [0, 0, 0];
  private accentInv: [number, number, number] = [0, 0, 0];
  ...
  private drawLetter() {
    ...
    const family = (this.p.fontFamily ?? "").split(",")[0].trim();
    let size = Math.round(H * (single ? 0.86 : 0.6));
    mctx.fillStyle = "oklch(1 0 0)"; // white alpha mask, tinted at composite time
  ...
  private buildGrain() {
    ...
    g.globalAlpha = 0.06;
    g.fillStyle = "oklch(0 0 0)"; // scanline shade
