...
export interface SquircleProps {
  ...
  className?: string;
  style?: CSSProperties;
  /** Fill colour or CSS gradient for the body (any CSS colour, e.g. `var(--card)`). */
  fill?: string;
  ...
  return (
    <div ref={ref} className={className} style={{ position: "relative", ...style }}>
      ...
      {w > 0 && h > 0 && (
        ...
        >
          {fill && !isCssGradient && <path d={body} style={{ fill }} />}
        </svg>
      )}
          ...
          {strokeGradient && (
            <linearGradient id={`sq-stroke-${uid}`} x1="0" y1="0" x2="0" y2="1">
              {strokeGradient.map((c, i) => (
                <stop key={i} offset={i / (strokeGradient.length - 1)} style={{ stopColor: c }} />
              ))}
            </linearGradient>
          )}
          <path
            d={body}
            fill="none"
            style={{ stroke: strokeGradient ? `url(#sq-stroke-${uid})` : stroke }}
