/**
 * Gyro Rings — Originkit (source supplied by the user), three.js scene. Ring colours come from
 * `--gyro-tint` (metal finish) and `--gyro-color` (solid finish), declared on the root with the
 * upstream values; `tint` / `color` props override them.
 */
import * as React from "react";
import { useEffect, useRef } from "react";
import * as THREE from "three";
import matcapUrl from "./polished-metal-matcap.png";
...
const DEFAULTS: Config = { rings: 6, finish: "metal", tint: "", color: "", thickness: 5, innerRadius: 40, gap: 13, spin: 2, hoverBoost: 10, dragSensitivity: 1, sizePercent: 104 };
/** CSS colour string → sRGB via a 1×1 canvas (three.js does not parse oklch). */
function cssColor(value: string) { const ctx = document.createElement("canvas").getContext("2d")!; ctx.fillStyle = value.trim(); ctx.fillRect(0, 0, 1, 1); const [r, g, b] = ctx.getImageData(0, 0, 1, 1).data; return new THREE.Color().setRGB(r / 255, g / 255, b / 255, THREE.SRGBColorSpace); }
...
class GyroScene {
  ...
  constructor(container: HTMLElement, cfg: Config) {
    ...
    const light = new THREE.DirectionalLight(new THREE.Color(1, 1, 1), .9); light.position.set(.4, .7, 1); this.camera.add(light); this.scene.add(new THREE.AmbientLight(new THREE.Color(1, 1, 1), .65), this.camera, this.root);
  ...
  /** Resolves the configured or declared CSS colours (any CSS syntax, incl. oklch) for three.js. */
  private paint() { const css = getComputedStyle(this.container); this.metal.color.set(cssColor(this.cfg.tint || css.getPropertyValue("--gyro-tint"))); this.solid.color.set(cssColor(this.cfg.color || css.getPropertyValue("--gyro-color"))); }
  ...
  update(c: Config) { const previous = this.cfg; this.cfg = c; this.paint(); if (c.rings !== previous.rings || c.thickness !== previous.thickness || c.innerRadius !== previous.innerRadius || c.gap !== previous.gap) this.build(); else if (c.finish !== previous.finish) this.ringList.forEach((r) => r.mesh.material = this.material()); this.cameraUpdate(); }
...
export type GyroRingsProps = Partial<Config> & { style?: React.CSSProperties; className?: string };
/** Nested rings spinning on alternating axes; drag to rotate (with inertia), hover to speed up. */
export function GyroRings(props: GyroRingsProps) {
  ...
  return <div ref={ref} role="img" aria-label="Gyroscope rings" className={["[--gyro-tint:oklch(0.882_0_0)] [--gyro-color:oklch(0.877_0.227_151.8)]", props.className].filter(Boolean).join(" ")} style={{ position: "relative", width: "100%", height: "100%", minWidth: 180, minHeight: 180, overflow: "hidden", ...props.style }} />;
}
