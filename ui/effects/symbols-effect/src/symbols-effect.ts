...
/** Any CSS colour (incl. oklch / var-resolved values) → THREE.Color via a 1×1 canvas. */
...
export interface SymbolsParams {
  ...
  bandColors: string[]; // 4 CSS colours, dark band → light band
  bandStops: number[]; // 5 stops → 4 luminance bands
  bandGlyphs: number[]; // 4 glyph indices (0 = empty)
  zoom?: number; // source zoom (1 = fill)
  bg: string; // background colour around a shrunk source (CSS colour)
}
...
export class SymbolsEffect {
  ...
  constructor(canvas: HTMLCanvasElement, p: SymbolsParams) {
    ...
    this.renderer.setClearColor(cssColor(p.bg), 1);
    ...
    const col = p.bandColors.map(cssColor);
