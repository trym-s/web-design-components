...
// Colours default to variables declared on the <svg> root (upstream values in oklch).
export const DEFAULT_EDITOR: EditorStyle = {
  accent: "var(--vector-accent)",
  arm: "var(--vector-arm)",
  anchorR: 4,
  handleR: 3.2,
  pointFill: "var(--vector-point)",
  fill: "var(--vector-accent)",
  fillOpacity: 0.08,
  stroke: "var(--vector-accent)",
  strokeWidth: 1.5,
  showRig: true,
  fillRule: "evenodd",
};
...
export function VectorEditor({
  ...
  return (
    <svg
      ...
      className={cn(
        "block touch-none select-none overflow-visible [--vector-accent:oklch(0.67_0.183_249.2)] [--vector-arm:oklch(0.768_0.052_249.4)] [--vector-point:oklch(1_0_0)]",
        className,
      )}
    ...
    >
      {/* the shape itself */}
      <path d={d} fillRule={s.fillRule} fillOpacity={s.fillOpacity} strokeWidth={s.strokeWidth} style={{ fill: s.fill, stroke: s.stroke }} />

      {s.showRig && (
        <g>
          {/* tangent arms */}
          <g style={{ stroke: s.arm }} strokeWidth={Math.max(0.75, s.strokeWidth * 0.6)}>
            {arms.map((a, k) => (
          ...
          {path.anchors.map((a, i) => (
            <g key={`a-${i}`} style={{ cursor: "grab" }} onPointerDown={(e) => startAnchor(e, i)}>
              ...
              <circle cx={a.p.x} cy={a.p.y} r={s.anchorR * 3} fillOpacity={0} />
              <circle cx={a.p.x} cy={a.p.y} r={s.anchorR} strokeWidth={1.25} style={{ fill: s.pointFill, stroke: s.accent }} />
            </g>
          ))}
        </g>
      )}
...
function Diamond({
  ...
  return (
    <g style={{ cursor: "grab" }} onPointerDown={onPointerDown}>
      <circle cx={cx} cy={cy} r={r * 3.5} fillOpacity={0} />
      <rect
        x={cx - r}
        y={cy - r}
        width={r * 2}
        height={r * 2}
        strokeWidth={1.1}
        style={{ fill, stroke, transform: "rotate(45deg)", transformOrigin: "50% 50%", transformBox: "fill-box" }}
      />
    </g>
  );
}
