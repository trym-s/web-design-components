# Channel Strip Knob Pan

Channel strip with a bipolar pan knob
...
- Framework: react
- Entry point: `upstream/examples/block-channel-strip-knob-pan.tsx`
...
- Provides: Channel Strip Knob Pan block
- Requires: React, Tailwind v4 with the shadcn tokens and a shadcn style (`cn-*` slots), `@audio-ui/react` 0.1.2
- Variants: block-channel-strip-knob-pan
- Upstream: Audio UI · channel-strip, knob, ui
- Preferred install: `npx shadcn@latest add @audio/block-channel-strip-knob-pan`
...
- `upstream/examples/block-channel-strip-knob-pan.tsx` — Channel strip with a bipolar pan knob · static: `static/block-channel-strip-knob-pan.html`
...
### BlockChannelStripKnobPan — `block-channel-strip-knob-pan.tsx`

- Props: `title`, `label` ("Pan"), `footer`, `value` (−100…100), `onValueChange`, `onValueCommit`, `min` −100, `max` 100, `size` `lg`, `formatValue` (`formatPan`: C, L n, R n).
- Layout: A centre-anchored pan knob: the arc grows left or right from 0.
