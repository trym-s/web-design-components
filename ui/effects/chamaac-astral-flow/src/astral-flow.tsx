/**
 * Astral Flow — Chamaac UI `registry/chamaac/astral-flow/astral-flow.tsx` (commit 345d79b) without Next.js.
...
export interface AstralFlowProps {
  className?: string;
  /** Animation speed multiplier. */
  speed?: number;
  /** Darkest base colour; also the container background while the canvas loads. */
  color1?: string;
  /** Mid-tone smoke colour. */
  color2?: string;
  /** Highlight wisp colour. */
  color3?: string;
  /** Lower bound of the breathing outward flow. */
  flowMin?: number;
  /** Upper bound of the breathing outward flow. */
  flowMax?: number;
}

function Effect({ speed, color1, color2, color3, flowMin, flowMax }: Required<Omit<AstralFlowProps, "className">>) {
...
const NOISE = `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E")`;
...
export default function AstralFlow({
  ...
  return (
    ...
    >
      <Canvas camera={{ position: [0, 0, 1] }} dpr={1} frameloop={reducedMotion ? "demand" : "always"}>
        <Effect speed={speed} color1={color1} color2={color2} color3={color3} flowMin={flowMin} flowMax={flowMax} />
      </Canvas>
      {/* Film grain */}
      <div className="pointer-events-none absolute inset-0 opacity-[0.4] mix-blend-overlay" style={{ backgroundImage: NOISE }} />
    </div>
  );
}
