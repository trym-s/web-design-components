/**
 * BlockChannelStripFaderHorizontal — Audio UI's `block-channel-strip-fader-horizontal` (see ../upstream/examples/block-channel-strip-fader-horizontal.tsx) with its content and state
...
export type BlockChannelStripFaderHorizontalProps = {
  label?: string;
  value: number;
...
export function BlockChannelStripFaderHorizontal({
  label = "Fader",
  value,
  onValueChange,
  onValueCommit,
...
}: BlockChannelStripFaderHorizontalProps) {
  const faderId = useId();
  const labelId = useId();
  const valueId = useId();
  return (
