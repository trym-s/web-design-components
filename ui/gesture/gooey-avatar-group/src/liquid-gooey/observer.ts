...
export class ObserveEngine {
  ...
  private getGroup: () => HTMLElement | null
  constructor(getGroup: () => HTMLElement | null) {
  ...
  private refreshMelt(item: Item): void {
    ...
    gradient.append(
      ...
      svg('stop', { offset: '35%', 'stop-color': 'white', 'stop-opacity': '0.95' }),
      svg('stop', { offset: '60%', 'stop-color': 'white', 'stop-opacity': '0.6' }),
      svg('stop', { offset: '82%', 'stop-color': 'white', 'stop-opacity': '0.25' }),
  ...
  private writeBlend(item: Item, dt: number): boolean {
    ...
    for (const entry of melt.entries) {
      ...
      const hole = `radial-gradient(circle at ${hx}px ${hy}px, oklch(1 0 0 / ${holeAlpha}) ${round(hd * 0.32)}px, oklch(1 0 0 / ${holeMid}) ${round(hd * 0.55)}px, white ${round(hd * 0.8)}px, white ${far}px)`
