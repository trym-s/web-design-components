...
Shader Test shader background (in the Chamaac registry source, not on the site).
...
- Framework: react
- Entry point: `ui/_sources/chamaac/registry/chamaac/shader-test/page.tsx`
- Nature: decorative; reuse the effect, motion and composition, adapt literal values to the target project.
- Added: 2026-09-25T08:24:04Z
- Curation: pending
- Use when: Shader Test shader background (in the Chamaac registry source, not on the site).
- Provides: Shader Test
- Requires: React and Tailwind v4
- Variants: default
- Upstream: Chamaac UI · source-only
- Local source fallback: `ui/_sources/chamaac/registry/chamaac/shader-test/page.tsx`
...
### Watery Noise — `watery-noise.tsx`

The registry's `shader-test` experiment (component `WateryNoise`): a centred vortex of ridged noise that rotates continuously, divided into `color`. Block-level: fills its parent and is at least 400 px tall.

- Props: `speed` (1), `color` (`#191970`), `detail` (1.5), `distortion` (3), `brightness` (1), `backgroundColor` (`#05050f`), `className`.
