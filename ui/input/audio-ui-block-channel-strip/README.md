...
Channel strip with fader, header, and footer
...
- Framework: react
- Entry point: `upstream/examples/block-channel-strip.tsx`
...
- `upstream/examples/block-channel-strip.tsx` — Channel strip with fader, header, and footer · static: `static/block-channel-strip.html`
...
### BlockChannelStrip — `block-channel-strip.tsx`

- Props: `title` ("Channel 1"), `label` ("Volume"), `footer` ("Output"), `value`, `onValueChange`, `onValueCommit`, `min` 0, `max` 100, `step` 1, `formatValue` (`n%`), `aria-label`.
- Layout: A vertical strip: title, label, a vertical fader, the formatted value, footer.
