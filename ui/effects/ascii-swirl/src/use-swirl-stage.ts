...
export interface StageConfig {
  ...
  word?: string;
  style?: FontStyle;
  /** Gradient color stops (any CSS colour). One stop = flat fill. */
  inkStops: string[];
  /** Color of the resolved logo word (CSS colour), independent of the swirl letters. */
  logoColor: string;
  gradient: boolean;
  ...
  /** Colour hot trail cells glow toward; omit for no color flare. */
  trailFlare?: string;
  ...
  useEffect(() => {
    ...
    function frame(time: number) {
      ...
      if (grid && atlas && buffers) {
        ...
        const stops = (c.gradient ? c.inkStops : c.inkStops.slice(0, 1)).map(cssToRgb01);
        ...
        const count = composeField({
          ...
          elapsed,
          paint,
          logo: cssToRgb01(c.logoColor),
          slotOf,
          trail: c.trail ? trail : undefined,
          trailStrength: c.trailStrength,
          trailFlare: c.trailFlare ? cssToRgb01(c.trailFlare) : undefined,
