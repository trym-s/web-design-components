...
- Framework: react
- Entry point: `upstream/examples/block-channel-strip-knob-multi.tsx`
...
- `upstream/examples/block-channel-strip-knob-multi.tsx` — Multiple knob channel strips in a row · static: `static/block-channel-strip-knob-multi.html`
...
### BlockChannelStripKnobMulti — `block-channel-strip-knob-multi.tsx`

- Props: `effects` (`{ id, label, value }[]`), `onValueChange(id, value)`, `onValueCommit(id, value)`, `footer` ("FX"), `min`, `max`, `step`, `formatValue`.
- Layout: One strip per effect side by side, each with its name, a large knob and its value.
