/**
 * Water Caustic — Chamaac UI `registry/chamaac/water-caustic/water-caustic.tsx` (commit 345d79b) without Next.js.
...
function ShaderPlane({ color }: { color: string }) {
  ...
  const uniforms = useMemo(
    () => ({ iTime: { value: 0 }, iResolution: { value: new THREE.Vector2() }, uColor: { value: new THREE.Color(color) } }),
    [color],
  );
...
export interface WaterCausticProps {
  className?: string;
  /** Light colour of the caustic ridges; the canvas is transparent, so whatever sits behind shows through. */
  color?: string;
}

/** Full-bleed transparent overlay: fills its nearest positioned ancestor and ignores the pointer. */
