...
- Provides: Player block
- Requires: React, Tailwind v4 with the shadcn tokens and a shadcn style (`cn-*` slots), `@audio-ui/react` 0.1.2
- Variants: block-player
- Upstream: Audio UI · player, queue
- Preferred install: `npx shadcn@latest add @audio/block-player`
...
### BlockPlayer — `block-player.tsx`

- Props: none; it reads `AudioPlayerProvider`.
- Layout: A stacked player: elapsed time, seek bar, remaining time; then previous / play / next, volume and the queue dialog.
