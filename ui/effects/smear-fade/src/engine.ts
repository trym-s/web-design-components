...
/** Parse any CSS colour (the palette comes from declared CSS variables) into 0..1 rgb plus its
 *  alpha, via a 1×1 canvas. Cached: palettes are re-resolved during crossfades. */
const colorCache = new Map<string, { rgb: RGB; a: number }>();
function parseColor(c: string): { rgb: RGB; a: number } {
  ...
  const out = { rgb: [r / 255, g / 255, b / 255] as RGB, a: al / 255 };
...
export class FadeMotion {
  ...
  private idleMix = 1;
  /** measured width of the word at 100px, to detect a real font swap */
  ...
  constructor(host: HTMLElement, palettes: Palette[], word: string, fontFamily?: string) {
    this.host = host;
