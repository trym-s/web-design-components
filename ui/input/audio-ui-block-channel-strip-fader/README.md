...
Channel strip with a minimal fader section
...
- Framework: react
- Entry point: `upstream/examples/block-channel-strip-fader.tsx`
...
- Curation: pending
- Use when: Channel strip with a minimal fader section
- Provides: Channel Strip Fader block
- Requires: React, Tailwind v4 with the shadcn tokens and a shadcn style (`cn-*` slots), `@audio-ui/react` 0.1.2
- Variants: block-channel-strip-fader
- Upstream: Audio UI · channel-strip, fader, ui
- Preferred install: `npx shadcn@latest add @audio/block-channel-strip-fader`
...
- `upstream/examples/block-channel-strip-fader.tsx` — Channel strip with a minimal fader section · static: `static/block-channel-strip-fader.html`
