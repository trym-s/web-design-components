...
Modern Smoke shader background (in the Chamaac registry source, not on the site).
...
- Framework: react
- Entry point: `ui/_sources/chamaac/registry/chamaac/modern-smoke/page.tsx`
- Nature: decorative; reuse the effect, motion and composition, adapt literal values to the target project.
- Added: 2026-09-25T08:24:04Z
- Curation: pending
- Use when: Modern Smoke shader background (in the Chamaac registry source, not on the site).
- Provides: Modern Smoke
- Requires: React and Tailwind v4
- Variants: default
- Upstream: Chamaac UI · source-only
- Local source fallback: `ui/_sources/chamaac/registry/chamaac/modern-smoke/page.tsx`
...
- **Any other stack** — `static/modern-smoke.html` is the rendered DOM against `ui/_sources/chamaac/styles.css`.
...
Upstream page: https://github.com/amarnathdhumal/chamaacui/tree/main/registry/chamaac/modern-smoke
...
### Modern Smoke — `modern-smoke.tsx`

Block-level panel (fills its parent, at least 400 px tall) of three counter-rotating fbm layers forming a smoke body with glowing wisps, plus two overlays: soft radial light pools tinted from the palette (`mix-blend-screen`, 18 %) and SVG grain (`mix-blend-soft-light`, 12 %).

- Props: `speed` (1.2), `density` (0.08), `baseColor` (`#05070a`, also the container colour), `midColor` (`#173432`), `highlightColor` (`#9ac6bf`), `className`.
