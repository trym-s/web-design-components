...
- Framework: react
- Entry point: `upstream/examples/block-queue.tsx`
...
- Provides: Queue block
- Requires: React, Tailwind v4 with the shadcn tokens and a shadcn style (`cn-*` slots), `@audio-ui/react` 0.1.2
- Variants: block-queue
- Upstream: Audio UI · queue, ui
- Preferred install: `npx shadcn@latest add @audio/block-queue`
...
- `upstream/examples/block-queue.tsx` — Queue controls · static: `static/block-queue.html`
...
Upstream page: https://github.com/ouestlabs/audio-ui/tree/main/apps/www/src/registry-audio/bases/base/components/queue/block-queue.tsx
...
### BlockQueue — `block-queue.tsx`

- Props: none; it reads `AudioPlayerProvider`.
- Layout: A right-aligned row of shuffle, repeat, queue preferences and the queue dialog.
