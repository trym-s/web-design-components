# Channel Strip Fader Slider

Channel strip with horizontal fader and inline label
...
- Framework: react
- Entry point: `upstream/examples/block-channel-strip-fader-slider.tsx`
...
- Curation: pending
- Use when: Channel strip with horizontal fader and inline label
- Provides: Channel Strip Fader Slider block
- Requires: React, Tailwind v4 with the shadcn tokens and a shadcn style (`cn-*` slots), `@audio-ui/react` 0.1.2
- Variants: block-channel-strip-fader-slider
- Upstream: Audio UI · channel-strip, fader, ui
- Preferred install: `npx shadcn@latest add @audio/block-channel-strip-fader-slider`
...
- `upstream/examples/block-channel-strip-fader-slider.tsx` — Channel strip with horizontal fader and inline label · static: `static/block-channel-strip-fader-slider.html`
...
Upstream page: https://github.com/ouestlabs/audio-ui/tree/main/apps/www/src/registry-audio/bases/base/components/channel-strip/block-channel-strip-fader-slider.tsx
...
### BlockChannelStripFaderSlider — `block-channel-strip-fader-slider.tsx`

- Props: `label`, `value`, `onValueChange`, `onValueCommit`, `min`, `max`, `step`, `formatValue`, `aria-label`.
- Layout: A horizontal fader with the label and the value together on a header row above it, like a settings slider.
