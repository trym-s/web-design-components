/**
 * Iridescent Windows — Chamaac UI `registry/chamaac/iridescent-windows/iridescent-windows.tsx` (commit 345d79b)
...
export interface IridescentWindowsProps {
  className?: string;
  /** Animation speed multiplier. */
  speed?: number;
  /** Zoom of the complex-plane grid — higher means more, smaller windows. */
  scale?: number;
  /** Mid tone the shader derives a dark and a light shade from; used only when `themeColors` is not given. */
  color?: string;
  /** Explicit [dark, mid, light] palette; wins over `color`. */
...
const DEFAULT_THEME_COLORS: [string, string, string] = ["#4c6663", "#6f8f8a", "#b8cbc8"];

const toVector3 = (color: THREE.Color) => new THREE.Vector3(color.r, color.g, color.b);

function buildThemeColors(color: string, themeColors?: [string, string, string]) {
  if (themeColors) return themeColors.map((value) => toVector3(new THREE.Color(value)));
...
function Effect({ speed, scale, color, themeColors }: { speed: number; scale: number; color: string; themeColors?: [string, string, string] }) {
  const material = useRef<THREE.ShaderMaterial>(null);
  const shaderColors = useMemo(() => buildThemeColors(color, themeColors), [color, themeColors]);
  ...
  useFrame((state) => {
    ...
    material.current.uniforms.uResolution.value.set(state.size.width * state.viewport.dpr, state.size.height * state.viewport.dpr);
  });
...
export default function IridescentWindows({
  ...
  return (
    <div className={cn("relative h-full min-h-[400px] w-full overflow-hidden", className)} style={{ backgroundColor }}>
      <Canvas camera={{ position: [0, 0, 1] }} dpr={[1, 2]} frameloop={reducedMotion ? "demand" : "always"}>
        <Effect speed={speed} scale={scale} color={color} themeColors={themeColors} />
      </Canvas>
    </div>
  );
}
