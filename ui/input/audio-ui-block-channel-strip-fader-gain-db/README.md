# Channel Strip Fader Gain Db

Channel strip with bipolar gain fader (-60 dB to +6 dB)
...
- Framework: react
- Entry point: `upstream/examples/block-channel-strip-fader-gain-db.tsx`
...
- Provides: Channel Strip Fader Gain Db block
- Requires: React, Tailwind v4 with the shadcn tokens and a shadcn style (`cn-*` slots), `@audio-ui/react` 0.1.2
- Variants: block-channel-strip-fader-gain-db
- Upstream: Audio UI · channel-strip, fader, ui
- Preferred install: `npx shadcn@latest add @audio/block-channel-strip-fader-gain-db`
...
- `upstream/examples/block-channel-strip-fader-gain-db.tsx` — Channel strip with bipolar gain fader (-60 dB to +6 dB) · static: `static/block-channel-strip-fader-gain-db.html`
...
### BlockChannelStripFaderGainDb — `block-channel-strip-fader-gain-db.tsx`

- Props: `label` ("Gain"), `value`, `onValueChange`, `onValueCommit`, `min` −60, `max` 6, `step` 1, `formatValue` (`+n dB` / `n dB`), `aria-label`.
- Layout: A gain fader in decibels with a signed readout.
