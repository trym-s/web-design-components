/**
 * Watery Noise — Chamaac UI `registry/chamaac/shader-test/page.tsx` (commit 345d79b) without Next.js.
...
export interface WateryNoiseProps {
  className?: string;
  /** Animation speed multiplier. */
  speed?: number;
  /** Glow colour; the shader divides it by the noise field, so dark saturated colours read best. */
  color?: string;
  /** Noise frequency — higher means finer filaments. */
  detail?: number;
  /** Swirl strength around the centre. */
  distortion?: number;
  /** Glow falloff — lower is brighter. */
  brightness?: number;
  /** Container colour behind the canvas. */
  backgroundColor?: string;
}

function Effect({ speed, color, detail, distortion, brightness }: Required<Omit<WateryNoiseProps, "className" | "backgroundColor">>) {
  ...
  return (
    <div className={cn("relative h-full min-h-[400px] w-full overflow-hidden", className)} style={{ backgroundColor }}>
      <Canvas camera={{ position: [0, 0, 1] }} dpr={1} frameloop={reducedMotion ? "demand" : "always"}>
        <Effect speed={speed} color={color} detail={detail} distortion={distortion} brightness={brightness} />
      </Canvas>
    </div>
  );
}
