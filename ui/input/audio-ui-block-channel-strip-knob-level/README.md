# Channel Strip Knob Level

Channel strip with a level knob
...
- Framework: react
- Entry point: `upstream/examples/block-channel-strip-knob-level.tsx`
...
- Provides: Channel Strip Knob Level block
- Requires: React, Tailwind v4 with the shadcn tokens and a shadcn style (`cn-*` slots), `@audio-ui/react` 0.1.2
- Variants: block-channel-strip-knob-level
- Upstream: Audio UI · channel-strip, knob, ui
- Preferred install: `npx shadcn@latest add @audio/block-channel-strip-knob-level`
...
- `upstream/examples/block-channel-strip-knob-level.tsx` — Channel strip with a level knob · static: `static/block-channel-strip-knob-level.html`
...
Upstream page: https://github.com/ouestlabs/audio-ui/tree/main/apps/www/src/registry-audio/bases/base/components/channel-strip/block-channel-strip-knob-level.tsx
...
### BlockChannelStripKnobLevel — `block-channel-strip-knob-level.tsx`

- Props: `title`, `label` ("Level"), `footer`, `value`, `onValueChange`, `onValueCommit`, `min` 0, `max` 100, `step`, `size` (`lg`), `formatValue`, `aria-label`.
- Layout: Title, label, a large knob, value, footer.
